...
void TriMesh::computeNormalsT(){
  ...
  glm::vec3 U , V , normVec;

  for(unsigned int t=0; t<_triangles.size(); ++t) {

      U.x = _vertices[_triangles[t][1]].x - _vertices[_triangles[t][0]].x ;
      U.y = _vertices[_triangles[t][1]].y - _vertices[_triangles[t][0]].y ;
      U.z = _vertices[_triangles[t][1]].z - _vertices[_triangles[t][0]].z ;

      V.x = _vertices[_triangles[t][2]].x - _vertices[_triangles[t][0]].x ;
      V.y = _vertices[_triangles[t][2]].y - _vertices[_triangles[t][0]].y ;
      V.z = _vertices[_triangles[t][2]].z - _vertices[_triangles[t][0]].z ;

      normVec = glm::normalize(glm::cross(U,V));
...
void TriMesh::computeNormalsV(float angle_threshold){
  ...
  for(unsigned int t=0; t<_triangles.size(); ++t) {
      for(unsigned int w=0; w<3; ++w) {
            n = _normalsT[t];
            compt = 1;
            for(unsigned int v=0; v<_triangles.size(); ++v) {
                    if( t!=w ){
                            for(int q=0; q<3; ++q){
                                    if(_triangles[t][w]==_triangles[v][q]){
                                            n += _normalsT[v];
                                            compt++;
                                    }
                            }
